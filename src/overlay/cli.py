"""Interface en ligne de commande d'Overlay."""
...
from overlay.config import Config, ConfigError, config_path, load_config
from overlay.runtime import Runtime, build_runtime
...
def commande_pair(config: Config, args: argparse.Namespace | None = None) -> int:
    from overlay.config import resolve_token, rotate_token
    from overlay.server.pairing import pairing_summary
...
def _creer_serveur(runtime: Runtime):
    ...
    try:
        ...
        raise SystemExit(
            ...
            "pip install 'overlay[server]'"
        ) from erreur

    from overlay.server.auth import AuthThrottle
...
def _annoncer_serveur(runtime: Runtime) -> None:
    from overlay.server.pairing import local_ip_addresses, pairing_url, with_token
    ...
    print(f"Serveur Overlay sur {schema}://{adresse}:{port}/")
    ...
    print("Astuce : « overlay pair » affiche un QR code a scanner.")
...
def _run_avec_overlay(runtime: Runtime, *, server: bool) -> int:
    ...
    fil = threading.Thread(target=thread_asyncio, name="overlay-async", daemon=True)
    fil.start()
